...
queue()
  .defer(d3.csv, '../timeline/data/time_itemtype.csv')
  .defer(d3.csv, '../timeline/data/time_fct.csv')
  .defer(d3.csv, '../timeline/data/itemtype_fct_en.csv')
  .defer(d3.csv, '../timeline/data/time_itemtype_type.csv')
  .defer(d3.csv, '../timeline/data/type_fct_en.csv')
  .defer(d3.csv, '../timeline/data/time_keywordsNames_en.csv')
  .defer(d3.csv, '../timeline/data/time_place_en.csv')
  .defer(d3.csv, '../timeline/data/time_affiliate_en.csv')
  .await(dataLoaded);
 ...
 nested.sort(function(a,b){ return d3.ascending(a.total, b.total); })
  ...
  var tip = d3.tip().offset([-5, 0]).attr('class', 'd3-tip')
    .html(function(d) {
      ...
      else return d3.format(",")(d.count).replace(/,/g,".");
    });
  ...
  var urlTable = { "Stichworte" : "keywords_fct", "Orte" : "place_fct", "Personen/Organisationen" : "affiliate_fct_role", }
  ...
  function drawWords(area,data,selected,timeRange){
    ...
    nestedWords = nestedWords.slice(0,30)
  ...
  var overlay = d3.select("#overlay svg");
    ...
    overlay.selectAll("g").data(infos)
      .enter()
        ...
        .each("end",function(d){
          ...
          text
            .attr("transform", "translate("+(p ? (-bb.width-8) : (4+4))+","+16+")")
